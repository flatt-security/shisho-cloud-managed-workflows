package policy.aws.logmetric.bucket_policy_changes

import data.shisho
import future.keywords

test_whether_log_metric_filter_and_alarm_exist_for_s3_bucket_policy if {
	# check if there is a log metric filter and alarm for S3 bucket policy changes
	count([d |
		decisions[d]
	...
	# check if there is not a log metric filter and alarm for S3 bucket policy changes
	count([d |
		decisions[d]
