...
decisions[d] {
	...
	allowed := has_trail_with_s3_read(account.cloudTrail.trails)

	d := shisho.decision.aws.s3.bucket_read_trail({
		"allowed": allowed,
		"subject": account.metadata.id,
		"payload": shisho.decision.aws.s3.bucket_read_trail_payload({"enabled": allowed}),
	})
}

has_trail_with_s3_read(trails) {
	trail := trails[_]
	has_data_event_selector(trail)
} else := false

has_data_event_selector(trail) {
	has_basic_selector_with_s3_read(trail.eventSelectors)
} else {
	has_advanced_selector_with_s3_read(trail.eventSelectors)
} else = false

has_basic_selector_with_s3_read(selectors) {
	# Find a selector that ....
	selector := selectors[_]
	selector.__typename == "AWSCloudTrailBasicEventSelector"

	# logs all read events ...
	allowed_types := ["ALL", "READ_ONLY"]
...
has_advanced_selector_with_s3_read(selectors) {
	...
	not contains_field_selector(selector.fieldSelectors, "readOnly", "false")
