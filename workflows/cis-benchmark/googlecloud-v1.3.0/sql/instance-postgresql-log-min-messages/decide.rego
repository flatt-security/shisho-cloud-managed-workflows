package policy.googlecloud.sql.instance_postgresql_log_min_messages

import data.shisho

allowed_states := ["warning"]

decisions[d] {
	project := input.googleCloud.projects[_]
	instance := project.cloudSql.instances[_]

	state := log_min_messages_state(instance.settings.databaseFlags)
	...
	# the 'warning' is recommended, but please update the `allowed_states` depending on your organization's logging policy such as `allowed_states := ["error", "warning"]`
	...
	d := shisho.decision.googlecloud.sql.instance_postgresql_log_min_messages({
		...
		"payload": shisho.decision.googlecloud.sql.instance_postgresql_log_min_messages_payload({"log_min_messages_state": state}),
	})
}

# the default value is "warning"
log_min_messages_state(database_flags) := flag.value {
