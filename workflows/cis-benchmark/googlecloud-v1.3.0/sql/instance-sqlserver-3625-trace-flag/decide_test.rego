...
test_whether_3625_trace_flag_is_off_for_sqlserver_instances_of_cloud_sql if {
	# check if `3625` is off for all SQL Server instances of Google Cloud SQL
	count([d |
		decisions[d]
	...
	# check if `3625` is not off for all SQL Server instances of Google Cloud SQL
	count([d |
		decisions[d]
