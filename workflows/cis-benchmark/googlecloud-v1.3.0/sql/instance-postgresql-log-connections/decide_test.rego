...
test_whether_log_connections_is_off_for_postgresql_instances_of_cloud_sql if {
	# check if `log_connections` is set to `on` for all PostgreSQL instances of Google Cloud SQL
	count([d |
		decisions[d]
	...
	# check if `log_connections` is not set to `on` for all PostgreSQL instances of Google Cloud SQL
	count([d |
		decisions[d]
