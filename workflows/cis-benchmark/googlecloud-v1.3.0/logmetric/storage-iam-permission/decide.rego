package policy.googlecloud.logmetric.storage_iam_changes
...
decisions[d] {
	...
	d := shisho.decision.googlecloud.logmetric.storage_iam_changes({
		...
		"payload": shisho.decision.googlecloud.logmetric.storage_iam_changes_payload({"cis_notification_implementations": paths}),
	})
}
...
patterns := ["resource.type=gcs_bucket AND protoPayload.methodName=\"storage.setIamPermissions\""]
