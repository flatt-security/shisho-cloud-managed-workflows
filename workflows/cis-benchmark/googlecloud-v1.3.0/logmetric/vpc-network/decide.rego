package policy.googlecloud.logmetric.vpc_network_changes
...
decisions[d] {
	...
	d := shisho.decision.googlecloud.logmetric.vpc_network_changes({
		...
		"payload": shisho.decision.googlecloud.logmetric.vpc_network_changes_payload({"cis_notification_implementations": paths}),
	})
}
...
patterns := ["resource.type=\"gce_network\" AND (protoPayload.methodName:\"compute.networks.insert\" OR protoPayload.methodName:\"compute.networks.patch\" OR protoPayload.methodName:\"compute.networks.delete\" OR protoPayload.methodName:\"compute.networks.removePeering\" OR protoPayload.methodName:\"compute.networks.addPeering\")"]
