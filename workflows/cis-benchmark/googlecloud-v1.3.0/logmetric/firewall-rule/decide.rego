package policy.googlecloud.logmetric.firewall_rule_changes
...
decisions[d] {
	...
	d := shisho.decision.googlecloud.logmetric.firewall_rule_changes({
		...
		"payload": shisho.decision.googlecloud.logmetric.firewall_rule_changes_payload({"cis_notification_implementations": paths}),
	})
}
...
patterns := ["resource.type=\"gce_firewall_rule\" AND (protoPayload.methodName:\"compute.firewalls.insert\" OR protoPayload.methodName:\"compute.firewalls.patch\" OR protoPayload.methodName:\"compute.firewalls.delete\")"]
