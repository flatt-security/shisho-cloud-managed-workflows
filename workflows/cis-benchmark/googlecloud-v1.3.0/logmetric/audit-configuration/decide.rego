package policy.googlecloud.logmetric.audit_config_changes
...
decisions[d] {
	...
	d := shisho.decision.googlecloud.logmetric.audit_config_changes({
		...
		"payload": shisho.decision.googlecloud.logmetric.audit_config_changes_payload({"cis_notification_implementations": paths}),
	})
}
...
patterns := ["protoPayload.methodName=\"SetIamPolicy\" AND protoPayload.serviceData.policyDelta.auditConfigDeltas:*"]
