...
decisions[d] {
	...
	d := shisho.decision.aws.s3.bucket_versioning_lifecycle_policy({
		"allowed": allow_if_excluded(lifecycle_policy_enabled(rules), bucket),
		"subject": bucket.metadata.id,
		"payload": shisho.decision.aws.s3.bucket_versioning_lifecycle_policy_payload({"lifecycle_rules": rules}),
	})
}

lifecycle_policy_enabled(rules) {
	rule := rules[_]
	rule.status == "ENABLED"
} else = false
