...
test_irregular_port_detected if {
	count([d |
		decisions[d]
		...
		{"securityGroups": [{
			...
			"name": "default",
			# The rule should review inbound rules for 0.0.0.0/0 only
			"ipPermissionsIngress": [{
				"protocol": "tcp",
				"fromPort": 32,
				"toPort": 100,
	...
	count([d |
		decisions[d]
		shisho.decision.is_allowed(d)
	]) == 2 with input as {"aws": {"accounts": [{"network": {"vpcs": [{"securityGroups": [
		{
			"metadata": {"id": "aws-vpc-secuirty-group|ap-northeast-1|vpc-06dc8a2abafd88888|sg-0440545548fc44444"},
			"name": "test-group-2",
			# The range is the same as the allowed range
			"ipPermissionsIngress": [{
				"protocol": "tcp",
				"fromPort": 100,
				"toPort": 105,
		...
		},
		{
			"metadata": {"id": "aws-vpc-secuirty-group|ap-northeast-1|vpc-06dc8a2abafd88888|sg-0440545548fc44444"},
			"name": "test-group-2",
			# The range is a subset of the allowed range
			"ipPermissionsIngress": [{
				"protocol": "tcp",
				"fromPort": 101,
				"toPort": 104,
		...
		with data.params as {"authorized_tcp_ports": [100, 101, 102, 103, 104, 105]}
